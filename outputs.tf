...
output "chainlink_ea_memory_db_address" {
  ...
  value       = local.create && var.cache_redis ? aws_memorydb_cluster.this[0].cluster_endpoint[0].address : ""
}

output "chainlink_ea_memory_db_port" {
  description = "Port number that the cluster configuration endpoint is listening on"
  value       = local.create && var.cache_redis ? aws_memorydb_cluster.this[0].cluster_endpoint[0].port : ""
}
